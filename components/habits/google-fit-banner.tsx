...
import { syncStepsFromGoogleFit } from '@/app/dashboard/actions'
import type { DailyLog } from '@/types/habit'

interface GoogleFitBannerProps {
  connected: boolean
  log: DailyLog
  today: string
...
export function GoogleFitBanner({ connected, log, today, onLogChange }: GoogleFitBannerProps) {
    ...
    return (
      <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-3 flex items-center justify-between animate-fade-up">
        <div className="flex items-center gap-2.5 text-white/45">
          <ActivityIcon />
          <span className="text-[12px] font-medium">Auto-sync steps via Google Fit</span>
        </div>
        <a
          href="/api/google-fit/auth"
          className="text-[11px] font-semibold uppercase tracking-[0.15em] text-[#C8A96E]/80 hover:text-[#C8A96E] transition-colors"
        >
          Connect
        </a>
      </div>
  ...
  function handleSync() {
    ...
    startTransition(async () => {
      try {
        const { steps } = await syncStepsFromGoogleFit(today)
        setLastSteps(steps)
        onLogChange({
          ...log,
  ...
  const stepDisplay = lastSteps !== null
    ? lastSteps.toLocaleString()
    : log.habit_step_source === 'google_fit' && log.habit_step_count !== null
